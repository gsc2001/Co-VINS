...
void imgs_callback(const sensor_msgs::ImageConstPtr &depth_img_msg) {
    ...
    // cv_bridge::CvImageConstPtr rgb_img_ptr;
...
int main(int argc, char **argv) {
    ...
    camera = camodocal::CameraFactory::instance()->generateCameraFromYamlFile(config_file);
            ...
            // message_filters::Subscriber<sensor_msgs::Image> depth_img_sub(nh, DEPTH_IMAGE_TOPIC, 1),
            //         rgb_image_sub(nh, RGB_IMAGE_TOPIC, 1);
    ...
    ros::Subscriber depthSub = nh.subscribe(DEPTH_IMAGE_TOPIC, 1000, imgs_callback);

    pcd_pub = nh.advertise<pcl::PointCloud<pcl::PointXYZRGB>>(node_namespace + "/dense_pcd", 1);
